...
const Conversation: z.ZodType<Conversation> = z
	.object({
	...
	.passthrough();
const PaginatedResults_for_Conversation: z.ZodType<PaginatedResults_for_Conversation> = z
	.object({ records: z.array(Conversation), total: z.number().int() })
	.passthrough();
...
const LocalizedPage: z.ZodType<LocalizedPage> = z
...
const ToolConfig = z.union([
	z
		...
		.passthrough(),
	z.object({ pages: z.array(Page), type: z.literal('learn') }).passthrough(),
	z
		.object({ survey_id: z.string(), survey_url: z.string(), type: z.literal('heyform') })
		.passthrough()
]);
const WorkflowStep: z.ZodType<WorkflowStep> = z
	.object({
		activation_rule: ActivationRule,
		created_at: z.string().datetime({ offset: true }),
		description: z.string(),
	...
	.passthrough();
const CreateWorkflowStep: z.ZodType<CreateWorkflowStep> = z
	.object({
		activation_rule: ActivationRule,
		description: z.string(),
		is_offline: z.boolean(),
	...
	.passthrough();
const PartialWorkflowStep: z.ZodType<PartialWorkflowStep> = z
	.object({
...
const UserProgress: z.ZodType<UserProgress> = z
	.object({
		created_at: z.string().datetime({ offset: true }),
		id: z.string().uuid(),
		status: ProgressStatus,
