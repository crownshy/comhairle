<script lang="ts">
	...
	import type { ApiError, ResourcePermission } from '@crownshy/api-client/api';

	const { data, params } = $props();
	const { streamedUsersAndPermissions } = $derived(data);

	const resourceId = $derived(params.conversation_id);
	const resourceType = 'conversation';
	const role = 'content_editor';
	const grantReason = 'Conversation editing';
	...
	async function attemptGrantUserPermission({ cancel }: { cancel: () => void }) {
		...
		const response = await tryCatchAsync<ResourcePermission, ApiError>(() =>
					...
					<Card.Root>
						...
						<Card.Content>
							<Table.Root>
								...
								<Table.Body>
									{#each usersAndPermissions.ok as user (user.id)}
										<Table.Row>
