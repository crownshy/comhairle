<script lang="ts">
	...
	function chooseFile(row: UploadRow, file: File | null) {
		...
		if (file.size > maxSize) {
			notifications.send({
				message: `File "${file.name}" exceeds ${maxSizeLabel} limit`,
				priority: 'ERROR'
			});
			return;
		}
	...
	$effect(() => {
		...
		const interval = setInterval(() => {
			invalidate(key('event/recordings'));
		}, 10 * Second);
	...
	<section class="flex flex-col gap-4">
		<div class="flex flex-col gap-1">
			...
			<p class="text-muted-foreground text-sm">
				Audio up to {maxSizeLabel} per file. Upload one recording per room — add a row for each.
			</p>
		</div>
