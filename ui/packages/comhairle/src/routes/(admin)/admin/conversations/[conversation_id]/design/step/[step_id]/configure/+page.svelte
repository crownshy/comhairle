<script lang="ts">
	...
	import { Ms } from '$lib/utils/units';

	const { data } = $props();
	const { conversation, step } = $derived(data);
	...
	function setDataProtocol(protocol: DataProtocol) {
		...
		debouncedUpdateRequired(boolFromProtocol(protocol), 'requestUserSharePermission');
	}
...
<div class="flex flex-col gap-6">
	...
	<div class="pt-4">
		...
		<div class="pt-4">
			{#await data.streamedAvailableDocuments}
			...
			{:then availableDocuments}
				...
				<TranslatableField
					...
					{primaryLocale}
					{supportedLanguages}
					availableDocuments={availableDocuments.ok ?? []}
	...
	<div class="border-border flex flex-col gap-4 border-t pt-6">
		<div class="flex items-center gap-2">
			<Switch
				checked={revisitable}
				onCheckedChange={(value) => debouncedUpdateRequired(value, 'canRevisit')}
			/>
		...
		<div class="flex items-center gap-2">
			<Switch
				checked={required}
				onCheckedChange={(value) => debouncedUpdateRequired(value, 'required')}
			/>
	...
	<div class="border-border border-t pt-6">
		...
		<AlertDialog.Root bind:open={deleteOpen}>
			<AlertDialog.Content>
				<AlertDialog.Header>
					<AlertDialog.Title
						>Delete “{nameSource.contents[primaryLocale] ||
							'this step'}”?</AlertDialog.Title
					>
