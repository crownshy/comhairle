<script lang="ts">
	...
	let { data } = $props();
	let { workflowSteps, workflowStats } = $derived(data);
...
<TabContent>
		...
		<div class="grid grid-cols-3 gap-10 overflow-x-auto">
			...
			<Card.Root class="flex-inline flex">
				...
				<Card.Content class="h-full">
					<div class="flex flex-col items-center justify-center">
						{#if (workflowStats?.stepStats.find((s) => s.id == step.id)?.completed ?? 0) > 0}
							<h1 class="pt-17.5 text-3xl">1 Minute</h1>
							<p>median</p>
						{:else}
							<h1 class="pt-17.5 text-center text-2xl">
				...
				<Card.Footer class="flex-col gap-2 text-sm">
					{#if (workflowStats?.stepStats.find((s) => s.id == step.id)?.completed ?? 0) > 0}
						Most users completed this step in 1 minute. The longest it took someone was
						3 minutes.
					{/if}
				</Card.Footer>
			</Card.Root>
	...
	<div class="grid w-full grid-cols-1 gap-10 md:grid-cols-2">
		...
		<div class="flex flex-col gap-4">
			<p>
				Download demographic data from user profiles for participants in this conversation
			</p>
			<Button
