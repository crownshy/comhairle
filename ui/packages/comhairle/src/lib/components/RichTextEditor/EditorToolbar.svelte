...
<div
	class="border-border bg-muted relative flex min-h-[3rem] items-center gap-1 overflow-x-auto rounded-t-[12px] border border-b-0 px-3 xl:p-2"
>
	...
	<div class="flex flex-1 items-center gap-1 xl:flex-none">
		...
		<div class="bg-border mx-1 h-5 w-px flex-shrink-0"></div>
		...
		{#if !compact}
			<button
				type="button"
				class="text-muted-foreground hover:bg-accent hover:text-accent-foreground ml-auto flex flex-shrink-0 cursor-pointer items-center justify-center rounded border-0 bg-transparent p-1.5 xl:hidden"
	...
	{#if !compact}
		<div
			class="bg-popover text-popover-foreground border-border absolute top-full right-0 left-0 z-10 hidden flex-col gap-2 border border-t-0 p-3 shadow-lg xl:static xl:flex xl:w-auto xl:flex-row xl:items-center xl:gap-1 xl:border-0 xl:bg-transparent xl:p-0 xl:shadow-none"
			class:flex={menuExpanded}
			class:hidden={!menuExpanded}
		>
			<div class="bg-border mx-1 hidden h-5 w-px flex-shrink-0 xl:block"></div>
