...
<div class="relative h-full w-full overflow-hidden rounded-xl">
	{#if loading}
		<div
			class="absolute inset-0 z-10 flex items-center justify-center bg-muted/50"
		>
			<div class="flex flex-col items-center gap-3">
				<div
					class="h-8 w-8 animate-spin rounded-full border-4 border-primary border-t-transparent"
				></div>
				<span class="text-sm text-muted-foreground">Connecting to meeting...</span>
			</div>
		</div>
	{/if}
	...
	{#if error}
		<div
			class="absolute inset-0 z-10 flex items-center justify-center bg-destructive/10"
		>
