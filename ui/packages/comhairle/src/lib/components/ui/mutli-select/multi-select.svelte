...
<div bind:this={containerRef} class="relative">
	<div
		class={cn(
			'border-input focus-within:border-ring focus-within:ring-ring/50 bg-background relative min-h-[38px] rounded-md border text-sm transition-[color,box-shadow] outline-none focus-within:ring-[3px]',
			selected.length > 0 && 'cursor-text p-1',
			...
			<div class="overflow-y-auto p-1">
				{#each selectables as option (option.value)}
					<button
						type="button"
						class={cn(
							'hover:bg-accent hover:text-accent-foreground flex w-full cursor-pointer items-center rounded-sm px-2 py-1.5 text-left text-sm outline-none',
