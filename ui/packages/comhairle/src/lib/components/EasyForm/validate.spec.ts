...
describe('Validate Schema', () => {
	it('validation should pass', () => {
		...
		expect(validate(formData, schema)).toEqual(
			typedObj<ReturnType<typeof validate>>({ ok: true, err: null })
		);
	});
	...
	it("Reject when a key doesn't exist", () => {
		...
		expect(validate(formData, schema)).toEqual(
			typedObj<ReturnType<typeof validate>>({ ok: null, err: 'KEY_NOT_FOUND' })
		);
	});
	...
	it('Reject when a file extension is not allowed', () => {
		...
		formData.append('file', file);
		expect(validate(formData, schema)).toEqual(
			typedObj<ReturnType<typeof validate>>({ ok: null, err: 'EXTENSION_NOT_ALLOWED' })
		);
	});

	it('Reject incorrect type', () => {
		const schema: Schema = {
			file: typedObj<InputAttr>({
				name: 'file'
			})
		};
		const formData = new FormData();
		...
		formData.append('file', file);
		expect(validate(formData, schema)).toEqual(
			typedObj<ReturnType<typeof validate>>({ ok: null, err: 'TYPE_WRONG' })
		);
	});
	...
	it('Reject too short', () => {
		...
		expect(validate(formData, schema)).toEqual(
			typedObj<ReturnType<typeof validate>>({ ok: null, err: 'TOO_SHORT' })
		);
	});
	...
	it('Reject too long', () => {
		...
		expect(validate(formData, schema)).toEqual(
			typedObj<ReturnType<typeof validate>>({ ok: null, err: 'TOO_LONG' })
		);
	});
	...
	it('Reject failed pattern', () => {
		...
		formData.append('email', 'test');
		expect(validate(formData, schema)).toEqual(
			typedObj<ReturnType<typeof validate>>({ ok: null, err: 'MATCH_FAILED' })
		);
	});
});
