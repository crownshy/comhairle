<script lang="ts">
	/** The one comhairle-specific file in this tool. Along with adapters/*, this is the only place allowed to import @crownshy/api-client. */
	...
	/** Keep refs in sync if props change after mount. */
	$effect(() => {
		adapterRefState.current = adapter;
		stepRefState.current = buildStepContext();
	});
