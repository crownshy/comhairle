...
export type PrioritizationAdapter = {
	...
	/** Dev-mode reset: will probably delete removes every response the calling user has submitted for the given proposal. Backed by DeleteMyProposalResponses on the real adapter; an in-memory no-op on the fake. */
	clearMyResponses(proposalId: string): Promise<void>;

	/** Drafts - local, no network */
