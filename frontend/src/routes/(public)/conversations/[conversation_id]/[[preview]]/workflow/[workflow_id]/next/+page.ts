...
export const load: PageLoad = async ({ parent, params }) => {
	let { api, conversation, preview } = await parent();
	...
	try {
		if (conversation.is_live) {
			...
			if (next_step) {
				redirect_url = workflow_step_url(conversation.id, workflow_id, next_step.id, preview)
			}
			else {
			...
			let steps = await api.ListWorkflowSteps({ params: { conversation_id: conversation.id, workflow_id: workflow_id } });
			let first_step = steps.find(s => s.step_order === 1);
