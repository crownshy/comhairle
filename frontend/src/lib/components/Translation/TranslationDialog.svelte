<script lang="ts">
  ...
  import { X, Sparkles, Check, MoreHorizontal, Search } from 'lucide-svelte';
  import * as DropdownMenu from '$lib/components/ui/dropdown-menu';
  import * as Popover from '$lib/components/ui/popover';
  import { Input } from '$lib/components/ui/input';
  import { Checkbox } from '$lib/components/ui/checkbox';
  import { allLanguages, type LanguageCode } from '$lib/config/languages';
  ...
  interface Props {
    ...
    onAutoSave?: (language: Language, content: string, status: TranslationStatus) => void;
  ...
  let activeLanguage = $state<Language | null>(null);
  ...
  let nonPrimaryLanguages = $derived(
    allLanguages.filter(lang => {
      const t = translations.find(tr => tr.language === lang.code);
      return !t || t.status !== 'primary';
    })
  );

  let filteredLanguages = $derived(
    nonPrimaryLanguages.filter(lang => 
      lang.name.toLowerCase().includes(languageSearch.toLowerCase())
    )
  );

  function isLanguageEnabled(code: Language): boolean {
  ...
  function setActiveLanguage(language: Language | null) {
    if (!language || primaryTranslation?.language === language) return;
    activeLanguage = language;
  }
  
  function updateContent(language: Language | null, content: string) {
    if (!language) return;
  ...
  function approveTranslation(language: Language | null) {
    if (!language) return;
    const index = translations.findIndex(t => t.language === language);
  ...
  function unapproveTranslation(language: Language | null) {
    if (!language) return;
    const index = translations.findIndex(t => t.language === language);
  ...
  function toggleLanguage(code: Language) {
    ...
    const isEnabled = isLanguageEnabled(code);

    if (isEnabled) {
      translations = translations.filter(t => t.language !== code);
      
      if (activeLanguage === code) {
        const nonPrimary = translations.find(t => t.status !== 'primary');
        if (nonPrimary) {
          activeLanguage = nonPrimary.language;
        }
      }
    } else {
      translations = [
    ...
    onLanguageToggle?.(code, !isEnabled);
  }
            ...
            {:else}
              ...
              <div class="self-stretch flex-1 flex flex-col items-center justify-center gap-4 min-h-[300px] bg-gray-50 rounded-md border-2 border-dashed border-gray-200">
                <div class="text-gray-400 text-center">
                  ...
                  <p class="text-sm mt-1">Click "+ Add new" above to add a language</p>
                </div>
              </div>
            {/if}
          </div>
